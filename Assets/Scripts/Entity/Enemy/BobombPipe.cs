...
public class BobombPipe : MonoBehaviourPun {
    ...
    void TryToShoot() {
        ...
        if (IntersectsPlayer((Vector2) transform.position - searchOffset, searchBox)) {
            GameObject newBill = PhotonNetwork.InstantiateRoomObject("Prefabs/Enemy/Bobomb", transform.position + new Vector3(-spawnOffset.x, spawnOffset.y), Quaternion.identity, 0, new object[]{ true });
            bills.Add(newBill);
            return;
        }
    }
